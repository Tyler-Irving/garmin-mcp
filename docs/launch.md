...
## 1. The demo GIF (the README's single biggest conversion lever)
...
**A sample terminal GIF already ships** (`docs/demo.gif`, generated by
`scripts/make_demo_gif.py` — sample data, no real numbers). It's a solid placeholder. The
**upgrade** is a real Claude Desktop recording, which shows the actual conversational
product — do that when you can; recipe below.
